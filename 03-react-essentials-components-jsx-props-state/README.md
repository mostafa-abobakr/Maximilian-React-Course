...
## 4. Working with Functions & Events

- **Where to define them:** * If a function needs access to the component's `state` or `props`, define it *inside\* the component (before the `return` statement).
...
- **Declarative vs. Imperative:** In standard Vanilla JS, we write imperative code (giving step-by-step instructions to manipulate the DOM). In React, we write **declarative** code. We define what the final UI should look like based on the current state, and React handles the DOM updates behind the scenes using a "Virtual DOM."

## 6. State and React Hooks
...
## 7. Conditional Rendering Tricks
